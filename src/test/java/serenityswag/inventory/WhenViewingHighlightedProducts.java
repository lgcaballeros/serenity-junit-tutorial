...
public class WhenViewingHighlightedProducts {
    ...
    public void shouldDisplayCorrectProductDetailsPage() {
        login.as(User.STANDARD_USER);
        String firstName = productList.titles().get(0);
        //productList.openProductDeatilsFor(firstName);
        viewProductDetails.forProductWithName(firstName);
        Serenity.reportThat("The product name should be correctly displayed",
                () -> assertThat(productDetails.productName()).isEqualTo(firstName)
        );
        Serenity.reportThat("The product image should have the correct alt text",
                () -> productDetails.productImageWithAltValueOf(firstName).shouldBeVisible()
        );
